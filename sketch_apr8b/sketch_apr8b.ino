...
const long intervaloCor = 500; // Lê o sensor a cada 500ms (ajuste se precisar de mais tempo)
...
void loop() {
  ...
  // --- FILTRO DE TEMPO PARA COR ---
  if (millis() - tempoAnteriorCor >= intervaloCor) {
    tempoAnteriorCor = millis(); 
    verde(); // Descomentado e controlado pelo tempo
  }
...
void verde() {
  ...
  if ((pulseGd != 0) && (pulseBd != 0) && (pulseRd != 0)) {
    // CORREÇÃO DA LÓGICA + MARGEM SOLICITADA:
    // Menor valor de pulso = cor mais intensa. 
    // Portanto, o Verde (pulseGd) tem que ser MENOR que Vermelho e Azul por uma margem de 5 pontos.
    if ((pulseGd < (pulseBd - 5)) && (pulseGd < (pulseRd - 5)) && (pulseGd > 20) && (pulseGd < 100)) {
      
      // Ação quando detecta verde real
...
void desvioE() {
  analogWrite(enable1, 0); digitalWrite(sentido1, LOW); digitalWrite(sentido2, LOW);
  analogWrite(enable2, 0); digitalWrite(sentido3, LOW); digitalWrite(sentido4, LOW);
  analogWrite(enable3, 0); digitalWrite(sentido5, LOW); digitalWrite(sentido6, LOW);
  analogWrite(enable4, 0); digitalWrite(sentido7, LOW); digitalWrite(sentido8, LOW);
  delay(1000);
  digitalWrite(enable1, HIGH); digitalWrite(sentido1, LOW); digitalWrite(sentido2, HIGH);
  digitalWrite(enable2, HIGH); digitalWrite(sentido3, HIGH); digitalWrite(sentido4, LOW);
  digitalWrite(enable3, HIGH); digitalWrite(sentido5, LOW); digitalWrite(sentido6, HIGH);
  digitalWrite(enable4, HIGH); digitalWrite(sentido7, LOW); digitalWrite(sentido8, HIGH);
  delay(800);
  digitalWrite(enable1, HIGH); digitalWrite(sentido1, HIGH); digitalWrite(sentido2, LOW);
  digitalWrite(enable2, HIGH); digitalWrite(sentido3, LOW); digitalWrite(sentido4, HIGH);
  digitalWrite(enable3, HIGH); digitalWrite(sentido5, LOW); digitalWrite(sentido6, HIGH);
  digitalWrite(enable4, HIGH); digitalWrite(sentido7, LOW); digitalWrite(sentido8, HIGH);
  delay(1600);
  digitalWrite(enable1, HIGH); digitalWrite(sentido1, HIGH); digitalWrite(sentido2, LOW);
  digitalWrite(enable2, HIGH); digitalWrite(sentido3, LOW); digitalWrite(sentido4, HIGH);
  digitalWrite(enable3, HIGH); digitalWrite(sentido5, HIGH); digitalWrite(sentido6, LOW);
  digitalWrite(enable4, HIGH); digitalWrite(sentido7, HIGH); digitalWrite(sentido8, LOW);
  delay(1000);
  digitalWrite(enable1, HIGH); digitalWrite(sentido1, HIGH); digitalWrite(sentido2, LOW);
  digitalWrite(enable2, HIGH); digitalWrite(sentido3, LOW); digitalWrite(sentido4, HIGH);
  digitalWrite(enable3, HIGH); digitalWrite(sentido5, LOW); digitalWrite(sentido6, HIGH);
